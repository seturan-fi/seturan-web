...
import type { HexAddress, TxStatus, BorrowParams } from "@/types/types.d";
...
export const useBorrow = () => {
  ...
  const mutation = useMutation({
    mutationFn: async ({ poolAddress, amount, decimals }: BorrowParams) => {
      try {
        if (!address) {
        ...
        setStatus("loading");
        toast.loading("Borrowing...", { id: BORROW_CONFIG.toastId });
